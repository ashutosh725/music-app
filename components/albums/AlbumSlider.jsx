...
if(!data) return 'Loading...'
  return (
   <>
   <Swiper
   effect={'coverflow'} speed={1000}  spaceBetween={80} allowTouchMove={true}
   thumbs={{swiper: thumbSwiper && !thumbSwiper.destroyed ? thumbSwiper :null,}} modules={[FreeMode, Navigation, Thumbs, EffectCoverflow]} coverflowEffect={{
    rotate: 50,
    stretch: 0,
    depth: 100,
    modifier: 1,
            ...
            return <SwiperSlide key={album.id} className="mb-12">
                <div className="w-full bg-secondary/80 rounded-[10px] flex flex-col xl:flex-row items-center p-6 xl:p-12 gap-x-12">
                    <div className="hidden xl:flex w-[300px] h-[300px] xl:w-[500px] xl:h-[500px] relative cursor-pointer rounded-[10px] overflow-hidden">
                        <Image src={album.img} fill priority alt="" className="object-contain"/></div>
                    <div className="flex flex-1 w-full h-[500px]">
                        <div className="flex-1 flex flex-col xl:px-12">
                            {album.tracks?.map((track, index)=>{
                                return <div className="flex flex-1 w-full h-[500px]" key={index}>
                                  ...
                                  <div className="text-sm xl:text-base ">  {track.name}</div>
                                    ...
                                    <div><AudioPlayer style={{background: 'transparent', boxShadow: 'none', width: '100%'}} src={track.src} loop preload="none" color="#fff" volume={40} volumePlacement="bottom" className="album-player"/></div>
   ...
   <Swiper onSwiper={setThumbSwiper}
   breakpoints={{
    320: {
        slidesPerView:2,
        spaceBetween: 10,
   ...
   spaceBetween={20} slidesPerView={5} freeMode={true} watchSlidesProgress={true} className="thumb-slider"
   >
